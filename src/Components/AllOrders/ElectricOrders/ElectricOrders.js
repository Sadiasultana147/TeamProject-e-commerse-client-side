...
const ElectricOrders = () => {
  ...
  useEffect(() => {
    fetch(`https://fast-everglades-79425.herokuapp.com/electronicCollection/${_id}`)
      .then((res) => res.json())
  ...
  const handleOrder = (e) => {
    ...
    const productName = electronics.name;
    const productImage = electronics.image;
    const productDetails = electronics.features;
    const price = electronics.price;
    ...
    >
      ...
      <div className="d-flex m-5">
        <div className="col-md-6 text-center">
          <h2>{electronics?.name}</h2>
          <div>
            <img src={electronics?.image} alt="" className="w-75 p-5" />
          </div>
        </div>
